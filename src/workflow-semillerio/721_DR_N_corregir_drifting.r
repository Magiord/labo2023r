...
drift_normaliza <- function(campos_drift) {
  for (campo in campos_drift){
    ...
    #media <- mean(dataset$campo, na.rm=TRUE)
...
#------------------------------------------------------------------------------
drift_escala <- function(campos_drift) {
  for (campo in campos_drift){
    cat(campo, " ")
    ...
    dataset[, paste0(campo, "_escala") := (get(campo) - min(get(campo), na.rm = TRUE)) / (max(get(campo), na.rm = TRUE) - min(get(campo), na.rm = TRUE)), by = .(foto_mes)]
...
drift_intvert <- function(campos_drift) {
  for (campo in campos_drift){
    ...
    dataset[, paste0(campo, "_intvert") := (get(campo) / sum(get(campo), na.rm=TRUE) *100), by = .(foto_mes)]
    
    # integración vertical de la variable utilizando data.table
    #dataset[, paste0(campo, "_intvert") := get(campo) / desvio * 100]
    #dataset[, (campo) := NULL]
  }
}
...
switch(PARAM$metodo,
       ...
       "escala"         = drift_intvert(campos_monetarios),
)
