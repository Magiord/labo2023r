...
#------------------------------------------------------------------------------
drift_normaliza <- function(dataset, campos_drift, foto_mes) {
  # Convertir el dataset a un data.table
 # setDT(dataset)
  
  for (campo in campos_drift){
    cat(campo, " ")
    
    # Agrupar el dataset por la variable especificada en grupo_var
    dataset[, (campo) := (get(campo) - mean(get(campo), na.rm = TRUE)) / sd(get(campo), na.rm = TRUE), by = {{foto_mes}}]
  }
